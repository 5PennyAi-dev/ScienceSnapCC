
import { Language } from './types';

export const translations = {
  en: {
    appTitle: "ScienceSnap",
    appSubtitle: "Learn. Visualize. Inspire.",
    gallery: "Gallery",
    heroTitlePrefix: "Turn",
    heroTitleHighlight: "Science",
    heroTitleMiddle: "into",
    heroTitleSuffix: "Visual Art",
    heroSubtitle: "Generate stunning infographics for education.",
    tabDomain: "Explore Domain",
    tabConcept: "Explain Concept",
    placeholderDomain: "E.g. Marine Biology, Astrophysics...",
    placeholderConcept: "E.g. Black Holes, Photosynthesis, DNA...",
    btnDiscover: "Discover",
    btnVisualize: "Visualize",
    backToInput: "Back",
    discoveriesIn: "Discoveries in",
    selectFact: "Select a fact to visualize.",
    factCardLabel: "Fact",
    btnCreateInfographic: "Create Infographic",
    resultTitle: "Your Infographic is Ready!",
    basedOn: "Based on:",
    aiPlan: "AI Generation Plan",
    btnRegenerate: "Regenerate",
    btnSave: "Save to Gallery",
    galleryTitle: "Your Gallery",
    btnCreateNew: "Create New",
    galleryEmpty: "Your gallery is empty. Create some amazing science infographics!",
    filterAll: "All",
    filterLabelDomain: "Domain",
    filterLabelAudience: "Audience",
    filterLabelStyle: "Style",
    filterLabelLanguage: "Language",
    searchGalleryPlaceholder: "Search title...",
    noResultsTitle: "No matches found",
    noResultsDesc: "Try adjusting your filters to see more results.",
    btnResetFilters: "Clear filters",
    modalDetails: "Details",
    modalDomain: "Domain",
    modalTitle: "Title",
    modalFact: "Scientific Fact",
    modalEditLabel: "Edit Infographic",
    modalStyle: "Style",
    modalAudience: "Audience",
    modalRatio: "Format",
    modalModel: "Model",
    modalLanguage: "Language",
    placeholderEdit: "e.g. Change background to deep ocean...",
    btnDownload: "Download Image",
    downloading: "Downloading...",
    applyingMagic: "Applying magic...",
    loadingConsulting: "Consulting scientific archives...",
    loadingResearching: "Researching and synthesizing data...",
    loadingProcessing: "Analyzing and formatting your fact...",
    loadingPlanning: "Designing a visual masterpiece...",
    loadingRendering: "Rendering infographic pixels...",
    loadingSaving: "Saving to Gallery...",
    errorGenFacts: "Something went wrong retrieving facts.",
    errorGenConcept: "Something went wrong explaining the concept.",
    errorProcessFact: "Could not process your fact.",
    errorGenImage: "Failed to create infographic.",
    errorSave: "Failed to save infographic.",
    audienceKids: "For Kids",
    audienceAdults: "Pro / Adult",
    modelFlash: "Flash",
    modelPro: "Pro (HD)",
    ratioSquare: "Square (1:1)",
    ratioPortrait: "Portrait (3:4)",
    ratioLandscape: "Landscape (4:3)",
    ratioTall: "Tall (9:16)",
    styleLabel: "Art Style",
    styleDefault: "Default",
    stylePixel: "Pixel Art",
    styleClay: "Claymation",
    styleOrigami: "Origami",
    styleWatercolor: "Watercolor",
    styleCyberpunk: "Cyberpunk",
    styleVintage: "Vintage",
    styleNeon: "Neon",
    styleManga: "Manga",
    styleGhibli: "Ghibli",
    btnFullScreen: "Full Screen",
    selectDomain: "Select a domain",
    domainAstrophysics: "Astrophysics & Space",
    domainMarineBiology: "Marine Biology & Oceans",
    domainDinosaurs: "Dinosaurs & Paleontology",
    domainAi: "AI & Robotics",
    domainHumanBody: "Human Body & Health",
    domainClimate: "Climate & Environment",
    domainQuantumPhysics: "Quantum Physics",
    domainVolcanology: "Volcanology & Geology",
    domainGenetics: "Genetics & Biotechnology",
    domainChemistry: "Chemistry & Reactions",
    domainRenewableEnergy: "Renewable Energy",
    domainInsects: "Insects & Entomology",
    conceptSuggestionsTitle: "Suggested Concepts",
    conceptSuggestionsLoading: "Loading suggestions...",
    conceptSuggestionsHeader: "Popular science concepts",
    processSuggestionsTitle: "Suggested Processes",
    processSuggestionsLoading: "Loading suggestions...",
    processSuggestionsHeader: "Popular science processes",
    exitFullScreen: "Exit Full Screen",
    tabProcess: "Process/Sequence",
    placeholderProcess: "E.g., Photosynthesis, Water Cycle, Butterfly Lifecycle...",
    loadingDiscoveringProcess: "Analyzing process structure...",
    loadingGeneratingStep: "Generating step",
    loadingPlanningStep: "Planning visual for step",
    loadingRenderingStep: "Rendering step",
    errorGenProcess: "Failed to generate process sequence.",
    processSteps: "Process Steps",
    allSteps: "All Steps",
    loadingMessages: [
      "Analyzing composition...",
      "Applying lighting effects...",
      "Refining details...",
      "Finalizing render...",
      "Polishing pixels...",
      "Adding scientific accuracy...",
      "Enhancing visual clarity..."
    ],
    loadingMessagesResearch: [
      "Consulting scientific archives...",
      "Verifying data sources...",
      "Synthesizing facts...",
      "Formatting results...",
      "Checking accuracy...",
      "Organizing information..."
    ]
  },
  fr: {
    appTitle: "ScienceSnap",
    appSubtitle: "Apprendre. Visualiser. Inspirer.",
    gallery: "Galerie",
    heroTitlePrefix: "Transformez la",
    heroTitleHighlight: "Science",
    heroTitleMiddle: "en",
    heroTitleSuffix: "Art Visuel",
    heroSubtitle: "Générez des infographies éducatives époustouflantes.",
    tabDomain: "Explorer un Domaine",
    tabConcept: "Expliquer un Concept",
    placeholderDomain: "Ex: Biologie Marine, Astrophysique...",
    placeholderConcept: "Ex: Trous Noirs, Photosynthèse, ADN...",
    btnDiscover: "Découvrir",
    btnVisualize: "Visualiser",
    backToInput: "Retour",
    discoveriesIn: "Découvertes en",
    selectFact: "Sélectionnez un fait à visualiser.",
    factCardLabel: "Fait",
    btnCreateInfographic: "Créer l'infographie",
    resultTitle: "Votre infographie est prête !",
    basedOn: "Basé sur :",
    aiPlan: "Plan de génération IA",
    btnRegenerate: "Régénérer",
    btnSave: "Enregistrer",
    galleryTitle: "Votre Galerie",
    btnCreateNew: "Créer Nouveau",
    galleryEmpty: "Votre galerie est vide. Créez des infographies scientifiques incroyables !",
    filterAll: "Tous",
    filterLabelDomain: "Domaine",
    filterLabelAudience: "Public",
    filterLabelStyle: "Style",
    filterLabelLanguage: "Langue",
    searchGalleryPlaceholder: "Rechercher un titre...",
    noResultsTitle: "Aucun résultat",
    noResultsDesc: "Essayez de modifier vos filtres pour voir plus de résultats.",
    btnResetFilters: "Effacer les filtres",
    modalDetails: "Détails",
    modalDomain: "Domaine",
    modalTitle: "Titre",
    modalFact: "Fait Scientifique",
    modalEditLabel: "Modifier l'infographie",
    modalStyle: "Style",
    modalAudience: "Public",
    modalRatio: "Format",
    modalModel: "Modèle",
    modalLanguage: "Langue",
    placeholderEdit: "Ex: Changer le fond en océan profond...",
    btnDownload: "Télécharger l'image",
    downloading: "Téléchargement...",
    applyingMagic: "Application de la magie...",
    loadingConsulting: "Consultation des archives scientifiques...",
    loadingResearching: "Recherche et synthèse des données...",
    loadingProcessing: "Analyse et formatage de votre fait...",
    loadingPlanning: "Conception d'un chef-d'œuvre visuel...",
    loadingRendering: "Rendu des pixels de l'infographie...",
    loadingSaving: "Enregistrement dans la galerie...",
    errorGenFacts: "Problème lors de la récupération des faits.",
    errorGenConcept: "Problème lors de l'explication du concept.",
    errorProcessFact: "Impossible de traiter votre fait.",
    errorGenImage: "Échec de la création de l'infographie.",
    errorSave: "Échec de l'enregistrement de l'infographie.",
    audienceKids: "Pour Enfants",
    audienceAdults: "Pro / Adulte",
    modelFlash: "Flash",
    modelPro: "Pro (HD)",
    ratioSquare: "Carré (1:1)",
    ratioPortrait: "Portrait (3:4)",
    ratioLandscape: "Paysage (4:3)",
    ratioTall: "Grand (9:16)",
    styleLabel: "Style Artistique",
    styleDefault: "Défaut",
    stylePixel: "Pixel Art",
    styleClay: "Pâte à modeler",
    styleOrigami: "Origami",
    styleWatercolor: "Aquarelle",
    styleCyberpunk: "Cyberpunk",
    styleVintage: "Vintage",
    styleNeon: "Néon",
    styleManga: "Manga",
    styleGhibli: "Ghibli",
    btnFullScreen: "Plein écran",
    selectDomain: "Choisir un domaine",
    domainAstrophysics: "Astrophysique & Espace",
    domainMarineBiology: "Biologie Marine & Océans",
    domainDinosaurs: "Dinosaures & Paléontologie",
    domainAi: "IA & Robotique",
    domainHumanBody: "Corps Humain & Santé",
    domainClimate: "Climat & Environnement",
    domainQuantumPhysics: "Physique Quantique",
    domainVolcanology: "Volcanologie & Géologie",
    domainGenetics: "Génétique & Biotechnologie",
    domainChemistry: "Chimie & Réactions",
    domainRenewableEnergy: "Énergies Renouvelables",
    domainInsects: "Insectes & Entomologie",
    conceptSuggestionsTitle: "Concepts suggérés",
    conceptSuggestionsLoading: "Chargement des suggestions...",
    conceptSuggestionsHeader: "Concepts scientifiques populaires",
    processSuggestionsTitle: "Processus suggérés",
    processSuggestionsLoading: "Chargement des suggestions...",
    processSuggestionsHeader: "Processus scientifiques populaires",
    exitFullScreen: "Quitter plein écran",
    tabProcess: "Processus/Séquence",
    placeholderProcess: "Ex: Photosynthèse, Cycle de l'eau, Métamorphose du papillon...",
    loadingDiscoveringProcess: "Analyse de la structure du processus...",
    loadingGeneratingStep: "Génération de l'étape",
    loadingPlanningStep: "Planification visuelle de l'étape",
    loadingRenderingStep: "Rendu de l'étape",
    errorGenProcess: "Échec de la génération de la séquence de processus.",
    processSteps: "Étapes du Processus",
    allSteps: "Toutes les Étapes",
    loadingMessages: [
      "Analyse de la composition...",
      "Application des effets de lumière...",
      "Raffinement des détails...",
      "Finalisation du rendu...",
      "Polissage des pixels...",
      "Ajout de la précision scientifique...",
      "Amélioration de la clarté visuelle..."
    ],
    loadingMessagesResearch: [
      "Consultation des archives scientifiques...",
      "Vérification des sources de données...",
      "Synthèse des faits...",
      "Mise en forme des résultats...",
      "Vérification de l'exactitude...",
      "Organisation des informations..."
    ]
  }
};

export const getTranslation = (lang: Language) => translations[lang];
