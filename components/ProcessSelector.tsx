  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        ...
        <span>{isLoading ? t.processSuggestionsLoading : t.processSuggestionsTitle}</span>
      ...
      {isOpen && !isLoading && suggestions.length > 0 && (
        <div className="absolute top-full left-0 mt-2 w-80 bg-white border-2 border-cyan-300 rounded-xl shadow-2xl overflow-hidden z-50 animate-fade-in">
          <div className="p-3 border-b-2 border-cyan-200 bg-gradient-to-r from-cyan-50 to-blue-50">
            <p className="text-xs font-bold uppercase tracking-widest text-cyan-700">{t.processSuggestionsHeader}</p>
          </div>
          <div className="p-2 max-h-80 overflow-y-auto">
            {suggestions.map((item, index) => (
              ...
              >
                <span className="font-bold text-gray-800 block mb-1">{item.process}</span>
